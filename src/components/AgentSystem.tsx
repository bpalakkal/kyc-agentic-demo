...
export const AGENTS: Agent[] = [
  ...
  { id: "nyse", name: "NYSE Listing", short: "NYSE", icon: BarChart2,
    description: "Checks NYSE/NASDAQ listing status through the SEC exchange association dataset.",
    defaultThoughts: ["Searching NYSE/NASDAQ listings…", "Checking listing status…", "Extracting ticker attributes…"] },
  { id: "nfa", name: "NFA BASIC", short: "NFA", icon: Scale,
    description: "Researches NFA BASIC for CFTC/NFA registrations, membership status, and categories.",
    defaultThoughts: ["Searching NFA BASIC…", "Reviewing registration categories…", "Extracting NFA attributes…"] },
  { id: "delaware", name: "State of Delaware", short: "Delaware", icon: Landmark,
    description: "Researches the Delaware Division of Corporations entity registry.",
    defaultThoughts: ["Searching Delaware entities…", "Reviewing registration record…", "Extracting entity attributes…"] },
  { id: "puerto-rico", name: "State of Puerto Rico", short: "Puerto Rico", icon: Landmark,
    description: "Researches the Puerto Rico Department of State corporation registry.",
    defaultThoughts: ["Searching Puerto Rico entities…", "Reviewing registration record…", "Extracting entity attributes…"] },
  { id: "us-sourcing-flow", name: "US Data Sourcing — All Sources", short: "US All", icon: Database,
    description: "Triggers all seven US source agents independently in parallel.",
    defaultThoughts: ["Querying seven US sources in parallel…", "Tracking independent source results…", "Ready for review"] },
...
const AGENT_CATEGORIES: AgentCategoryDef[] = [
  ...
  },
  {
    ...
    agentIds: ["iapd", "sec", "nyse", "nfa", "delaware", "puerto-rico", "gleif"],
  },
  {
    id: "screening",
    label: "Screening",
...
const AGENT_API_CONFIGS: Partial<Record<AgentId, AgentApiConfig>> = {
  ...
  ...Object.fromEntries(
    ["nfa", "delaware", "puerto-rico"].map((slug) => [slug, {
      slug,
      endpoint: `/api/agent-run/api/${slug}`,
      buildBody: (ctx: EntityCtx | null) => ({ entityName: ctx?.name ?? "", kycRef: ctx?.kyc ?? "" }),
      fetchSteps: true,
      asyncMode: true,
      apiRunner: true,
      skipSnapshot: true,
