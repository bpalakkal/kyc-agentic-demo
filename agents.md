...
### Current state: migrations 019-030

The authoritative migration sequence now runs through
`030_full_kyc_refresh.sql`. In addition to the earlier
migrations listed below:

- `019_source_person_lineage.sql` adds source-scoped person persistence.
- `020_sourcing_behavior_contract.sql` formalizes sourcing outcomes and evidence.
- `021_document_processing_flow.sql` adds post-sourcing classification and digitization.
- `022_customer_documents_and_digitizers.sql` adds customer uploads and concrete digitizers.
- `023_atomic_exception_number_allocation.sql` makes exception numbering concurrency-safe.
- `024_source_dd_sequence_guard.sql` prevents sourcing and DD from overlapping.
- `025_work_queue_agent_batches.sql` adds durable multi-case agent batches.
- `026_normalized_exception_assessments.sql` adds multi-value exception assessments and stable workflow linkage.
- `027_agent_model_profiles.sql` adds controlled Bedrock profiles.
- `028_exception_routing_agent.sql` registers the post-DD routing agent.
- `029_anthropic_model_profiles.sql` adds direct Anthropic profiles and the audited bulk provider switch.
- `030_full_kyc_refresh.sql` adds sequential UK and US full-refresh orchestrators.

`dist/` is generated by the GitHub Pages workflow and must not be committed.
Runtime `*.log` files are also ignored.

### Reboot handoff — 2026-07-23

- `main` contains the schema update and schema-driven UI capability through
  commit `b73228f`; the working tree was clean after push.
- The generated schema version is `a0c73bc14b619b34`: 131 attributes,
  11 collections, and 19 enums.
- Required case-creation fields are `entity_name`, `case_id`, `entity_id`,
  `policy`, and `risk_rating`.
- `regulator` is a repeatable non-person group containing `regulator`,
  `regulator_registration_number`, and `regulatory_status`.
- The reusable renderer is
  `src/components/kyc/SchemaDrivenForm.tsx`. Attribute View overrides also use
  generated control metadata.
- Last validation: TypeScript passed, 9 test files / 25 tests passed, and the
  production Vite build passed.
- `scripts/seed-demo-showcase.mjs` remains intentionally local-only and ignored.
...
npm run schema:update # Validate schemas and regenerate the UI contract
```
...
### Document classification and digitization

`document-processing-flow` is a dependency-only lifecycle wrapper. It claims
pending documents, classifies each document, selects an enabled
`agent_kind = 'document_digitizer'` registry row, and starts a child
`agent_run` using the document-specific slug. Examples include
`digitize-passport`, `digitize-sec-form-adv`, and
`digitize-certificate-of-incorporation`.

The wrapper remains in `agent_runs` for auditability. Dashboard frequency and
recent activity exclude `agent_kind = 'document_flow'`; concrete digitizer
child runs are the reporting unit. Classification is currently an internal
operation rather than a separately persisted agent.

### Work Queue batches and sequencing
...
### Exception assessment and workflow contract

- The current assessment lives on `entity_attributes`: `exception_flag` plus
  JSON arrays `exception_type`, `exception_reason`, and
  `exception_recommendation`.
- The durable lifecycle lives in `exceptions`: status, severity,
  `exception_types`, reasoning, recommended actions, sources, resolution data,
  and audit history.
- Workflow rows link to `entity_attribute_id` or `entity_person_id` when
  available.
- Agent exceptions synchronize into the matching attribute assessment during
  publishing. Clearing an assessment must not delete history; resolve or
  supersede the workflow row.
- Different open types on one attribute are distinct workflow items.
...
- `schema/kyc_master_attribute_schema.json` and `schema/screening_results_schema.json` are canonical. **Never hand-edit generated schema metadata or types.**
- Run `npm run schema:update` after replacing either schema. It validates both files and regenerates the versioned UI model and TypeScript declarations.
- UI controls, enums, defaults, required fields, exception controls, and repeatable collections must come from the generated model instead of new hard-coded field lists.
...
### Synthetic reset and local showcase data

Use `node scripts/seed-ria-test-cases.mjs` for a dry run and add `--execute` to
replace matching synthetic RIA cases with clean onboarding rows. The script
removes dependent case data and Storage objects, seeds 49 unique cases, and
verifies that prior snapshots, attributes, exceptions, runs, files, screening
records, overrides, and review cursors are absent.

`scripts/seed-demo-showcase.mjs` is intentionally local-only and untracked. It
creates disposable mock showcase data; remove that entity with
...
Migrations 010, 015, 021, and 022 seed the standard groups plus the
document-processing wrapper and concrete digitizers. Supabase is the golden
source for metadata, enablement, ordering, trigger behavior, execution mode,
agent kind, document type, and required environment variables. The API
enriches each row with runtime readiness:
- **Sourcing (12)**
